...
function header(active){return `<a class="skip-link" href="#main">Skip to content</a><header class="header"><div class="header-inner wrap"><a class="name" href="/index.html" aria-label="${escape(site.name)} home">${escape(site.namePrimary)} <span>${escape(site.nameSuffix)}</span></a><div class="header-tools">${navigation(active)}</div></div></header>`;}
function footer(active){return `<footer class="site-footer"><div class="wrap"><div class="footer-top"><div><p class="eyebrow">Contact</p><a class="footer-contact" href="mailto:${escape(site.footerEmail)}">${escape(site.footerEmail)}</a></div>${navigation(active,true)}</div><div class="footer-bottom"><p>© ${new Date().getFullYear()} ${escape(site.name)}</p><p>${escape(site.discipline)}</p></div></div></footer>`;}
function page({title,description,url,active,body,extra='',lang=site.lang}){return `<!doctype html>
<html lang="${escape(lang)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(title)} — ${escape(site.name)}</title><meta name="description" content="${escape(description)}"><meta name="theme-color" content="#ffffff"><meta name="color-scheme" content="only light"><link rel="canonical" href="${escape(site.origin)}${url}"><meta property="og:title" content="${escape(title)} — ${escape(site.name)}"><meta property="og:description" content="${escape(description)}"><meta property="og:type" content="website"><meta property="og:url" content="${escape(site.origin)}${url}"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><script src="/theme.js"></script><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link rel="stylesheet" href="/site.css"><script src="/site.js" defer></script>${extra}</head><body>${header(active)}${body}${footer(active)}</body></html>\n`;}
...
for (const [index,p] of projects.entries()) {
  ...
  const introHeading = p.bodyBlocks?.[0]?.type === 'heading' && p.bodyBlocks[0].depth === 2 ? p.bodyBlocks[0] : null;
  let imageIndex=0;
  const projectContent=p.bodyBlocks?p.bodyBlocks.filter(block=>block!==introHeading).map(block=>{
  ...
  const body=`<main id="main" class="project-main wrap" tabindex="-1"><a class="back-link project-back" href="/index.html"><span aria-hidden="true">←</span> All works</a><div class="project-heading${description?'':' project-heading--solo'}"><div><p class="eyebrow">${escape(p.brand)} / ${p.startedAt.slice(0,4)}</p><h1>${escape(p.model)}</h1></div>${description||introHeading?`<div class="project-summary">${introHeading?`<h2 class="project-product-title">${escape(introHeading.text)}</h2>`:''}${description?`<p class="intro">${escape(description)}</p>`:''}</div>`:''}</div><dl class="project-meta"><div><dt>Client</dt><dd>${escape(p.brand)}</dd></div><div><dt>Scope</dt><dd>${p.scope.map(escape).join('<br>')}</dd></div><div><dt>Year</dt><dd>${p.startedAt.slice(0,4)}</dd></div></dl><section class="project-content" aria-label="${escape(p.model)} project content">${projectContent}</section><a class="next-project" href="/${next.slug}/"><span><small>Next project / ${escape(next.brand)}</small><strong>${escape(next.model)}</strong></span><span aria-hidden="true">→</span></a></main><dialog class="viewer" id="image-viewer" aria-labelledby="viewer-title"><div class="viewer-toolbar"><p id="viewer-title">Project image</p><div class="viewer-actions"><button class="viewer-zoom" type="button" aria-pressed="false">Original size</button><button class="viewer-close" type="button" aria-label="Close image viewer">×</button></div></div><div class="viewer-stage" aria-busy="false"><img alt=""><p class="viewer-status" role="status" hidden></p></div></dialog>`;
