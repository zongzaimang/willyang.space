...
export function validateProject(root, p, complete = p.status !== 'draft' && p.status !== 'archived') {
  ...
  if (!complete) return;
  for (const key of ['brand', 'model', 'description']) text(p[key], `${p.id}.${key}`);
  ...
  for (const item of [p.cover, ...p.images]) {
    if (!item || !/^assets\/.+\.(png|webp|jpe?g)$/i.test(item.file ?? '')) throw new Error(`${p.id}: invalid image path`);
...
export function loadContent(root) {
  ...
  for (const key of ['name', 'namePrimary', 'nameSuffix', 'discipline', 'email', 'origin', 'lang']) text(site[key], `site.${key}`);
  ...
  const folder = path.join(root, 'content/projects');
  const projects = fs.readdirSync(folder, {withFileTypes:true}).filter(e => e.isDirectory()).map(e => {
    const p = readJSON(within(root, `content/projects/${e.name}/project.json`));
    if (p.id !== e.name) throw new Error(`Project folder does not match ID: ${e.name}`);
